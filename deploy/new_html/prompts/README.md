...
`utils/scriptPromptStandards.ts` 统一维护人物去重及出场规划规则，供剧本生成、修改、镜头拆分/合并、单镜头重生成和画面提示词提取复用：

- 同一角色在同一镜头中只出现一个实体。四视图和同一角色的多张参考图是身份参考，不代表多人。
- 跨镜头保持角色一致，但不同角色不得复制脸、混脸或互换身份。
- 剧情人数多于已有参考角色时只补足缺口，生成外貌不同的新背景角色，不复制已有角色凑人数。
- 新建或获准修改的镜头明确可见名单、人数、位置和动作；画外音不计入人数。“数名队员”等模糊人数在规划阶段明确，用稳定代号区分未具名人物。已明确的名单、人数和剧情关系不得擅自改动。
- 镜头合并后仍逐镜头遵守各自出场名单，不按参考图数量或多个镜头名单的并集生成同屏人物；无人镜头不添加人物。
- 分身、镜面倒影、双胞胎等仅在剧本或用户明确要求时作为例外，并遵守指定数量与身份。

规则属于生成默认要求，不批量重写历史剧本、锁定镜头或用户手动提示词。视频提交边界为新旧卡片的非空提示词补齐人物去重规则，重复提交不叠加；原可编辑文本不变，任务记录保留实际提交文本。提取阶段只继承已明确的人数，不补造名单；不更改素材绑定、生成计费或重试行为。提示词能降低重复人物概率，但不能保证模型输出完全合规，成片仍需检查。
