...
export const CHARACTER_UNIQUENESS_RULE =
  '人物去重约束：同一角色在同一镜头内只出现一个实体；人物四视图及同一角色的多张参考图只表示同一个人，不得生成为多人或把多视图拼进画面。不同角色分别保持各自的脸型、五官、发型和身份，不复制脸、不混脸、不相互替换；同一角色跨镜头保持一致，不等于不同角色长相相同。严格按本镜头明确的出场名单、可见人数及进出画动作生成，人数不按参考图数量计算；无人物镜头不添加人物。剧情所需人数多于已有独立参考角色时，仅补足人数缺口：生成具有不同脸型、五官、发型、体型的新背景角色，不能复制已有角色来凑人数，不能只换衣服保留同一张脸。只有剧本或用户明确要求时才允许分身、镜面倒影或双胞胎等例外，并遵守指定数量和身份。';

/** Submission-only default: preserve the editable script and apply to historical cards too. */
export function ensureVideoCharacterUniqueness(prompt: string): string {
  prompt = String(prompt || '');
  if (!prompt.trim() || prompt.includes(CHARACTER_UNIQUENESS_RULE)) return prompt;
  return `${prompt}\n\n【同镜头人物数量与身份约束】${CHARACTER_UNIQUENESS_RULE}合并片段逐镜头分别执行，不把各镜头的出场名单累加成同屏人数；未写明人数的群体采用满足剧情的最少合理人数。`;
}
