...
const select = vi.fn();
const refresh = vi.fn();
function Harness({ busy = false, selected = false }: { busy?: boolean; selected?: boolean }) {
  ...
  return <ProjectMaterialPicker {...picker} hideShotFilters busy={busy}
    ...
    handleAddProjectMaterial={select} onClose={() => {}} onRefresh={refresh} />;
}
afterEach(() => { cleanup(); vi.clearAllMocks(); });

describe('shared project material picker', () => {
  it('reuses categories, counts, search and original-image selection for blank video cards', () => {
    render(<Harness />);
    ...
    fireEvent.click(screen.getByTitle('添加 阿亮'));
  ...
  it('blocks extra selections at the reference limit', () => {
    ...
    expect(screen.getByTitle('添加 茶馆')).toBeDisabled();
